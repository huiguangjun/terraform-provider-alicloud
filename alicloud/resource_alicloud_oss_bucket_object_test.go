...
func TestAccAlicloudOssBucketObject_sse(t *testing.T) {
	tmpFile, err := ioutil.TempFile("", "tf-oss-object-test-acc-source")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccAlicloudOssBucketObject_sse1(t *testing.T) {
	tmpFile, err := ioutil.TempFile("", "tf-oss-object-test-acc-source")
	if err != nil {
		t.Fatal(err)
	}
	...
	rand := acctest.RandIntRange(1000000, 9999999)
	name := fmt.Sprintf("tf-testacc-object-%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourceOssBucketObjectConfigWithSseDependence)
...
func resourceOssBucketObjectConfigWithSseDependence(name string) string {
...
resource "alicloud_oss_bucket" "default" {
        ...
        server_side_encryption_rule {
                sse_algorithm     = "KMS"
                kms_master_key_id = alicloud_kms_key.key.id
        }
}
locals {
